function loadDoorHistory(id) {
	$.ajax({
		...
		success: function (history) {
			$('#door-history-error').hide();
			$('#door-history-table').show();

			const $tbody = $('#door-history-table tbody');
			$tbody.empty();
		...
		error: function (xhr) {
			...
			$('#door-history-table').hide();

			if (xhr.status === 404) {
				$('#door-history-error').text('Door not found.').show();
			} else {
				$('#door-history-error').text('Failed to load door history.').show();
			}
...
window.doorHistoryInterval = setInterval(() => loadDoorHistory(doorId), 1000);

loadDoorHistory(doorId);
